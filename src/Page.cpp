...
{
  uint16_t headerLength = sizeof(uint16_t) * 2 + numOfEntry * sizeof(uint16_t) * 2;
...
{
  int pos =
      pageNumber * pageSize + sizeof(uint16_t) * 2 + numOfEntry * sizeof(uint16_t) * 2;
  stream.seekp(pos);
...
void Page::deleteRow(int entryNum){
  int pos = sizeof(uint16_t) * 2 + entryNum * sizeof(uint16_t) * 2;
  stream.seekp(pos);
  stream.put(0);
  stream.put(0);
}
